...
## 지금 바로 되는 임시 베타 방식
- Firebase `이메일/비밀번호` 로그인이 아직 꺼져 있어도, 앱 안에서 브라우저 저장형 베타 계정으로 바로 가입/로그인 가능
- 이 임시 방식은 **현재 브라우저 안에서만** 계정이 유지됨
- 환자 홈 / 요청함 / 문진 / 방문기록 흐름 테스트는 계속 가능

## 진짜 Firebase 계정 베타로 가려면
- 친구가 다른 브라우저나 다른 컴퓨터에서도 같은 계정으로 로그인하려면
- Firebase 콘솔에서 `Authentication -> Sign-in method -> 이메일/비밀번호`를 켜야 함
